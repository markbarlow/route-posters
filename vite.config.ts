...
/**
 * GitHub Pages has no server-side rewrites, so a direct hit on /route-posters/create would 404
...
// `base` must match the GitHub Pages project path (https://<user>.github.io/route-posters/).
...
export default defineConfig({
  base: process.env.VITE_BASE ?? '/route-posters/',
