...
const BASE = '/route-posters/';

describe('toRoute', () => {
  it('maps the deployed paths onto routes', () => {
    expect(toRoute('/route-posters/', BASE)).toBe('/');
    expect(toRoute('/route-posters/create', BASE)).toBe('/create');
    expect(toRoute('/route-posters/samples', BASE)).toBe('/samples');
  });
  ...
  it('falls back to the splash for anything unrecognised', () => {
    ...
    expect(toRoute('/route-posters/create/deeper', BASE)).toBe('/');
    expect(toRoute('/route-posters/CREATE', BASE)).toBe('/');
  });

  it('does not mistake a path that merely starts with the base name', () => {
    expect(toRoute('/route-posters-old/create', BASE)).toBe('/');
  });
});

describe('toHref', () => {
  it('re-adds the base', () => {
    expect(toHref('/', BASE)).toBe('/route-posters/');
    expect(toHref('/create', BASE)).toBe('/route-posters/create');
  });
